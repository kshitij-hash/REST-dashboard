...
export default function EditStudentForm({ id, onClose, handleFormSubmit }: EditFormProps) {
    ...
    // Fetch student data and set it in the form
    useEffect(() => {
        const getData = async () => {
            try {
                const supabase = createClient();
                const { data: studentData, error } = await supabase.from('Students').select('*').eq('id', id).single();
                ...
                form.reset({
                    ...
                    subjects: studentData?.subjects.split(', '),
                    phone: studentData?.phone,
                    whatsapp: studentData?.whatsapp,
                    date_of_joining: new Date(studentData?.date)
                })
            } catch (error) {
    ...
    async function onSubmit(data: z.infer<typeof FormSchema>) {
        ...
        try {
            ...
            if (updateError?.message) {
                toast({
                    title: "Failed to update student record",
